...
class basic_group<Entity, exclude_t<Exclude...>, get_t<Get...>> {
    ...
    template<typename Func, typename... WeakRef>
    void traverse(Func func, type_list<WeakRef...>) const {
        for(const auto entt: *handler) {
            if constexpr(std::is_invocable_v<Func, decltype(get<WeakRef>({}))...>) {
                func(std::get<pool_type<WeakRef> *>(pools)->get(entt)...);
            } else {
                func(entt, std::get<pool_type<WeakRef> *>(pools)->get(entt)...);
            }
        }
    }
...
class basic_group<Entity, exclude_t<Exclude...>, get_t<Get...>, Owned...> {
    ...
    template<typename Func, typename... Strong, typename... WeakRef>
    void traverse(Func func, type_list<Strong...>, type_list<WeakRef...>) const {
        ...
        for(auto next = *length; next; --next) {
            if constexpr(std::is_invocable_v<Func, decltype(get<Strong>({}))..., decltype(get<WeakRef>({}))...>) {
                if constexpr(sizeof...(WeakRef) == 0) {
                    func(*(std::get<component_iterator<Strong>>(it)++)...);
                } else {
                    const auto entt = *(data++);
                    func(*(std::get<component_iterator<Strong>>(it)++)..., std::get<pool_type<WeakRef> *>(pools)->get(entt)...);
                }
            } else {
                const auto entt = *(data++);
                func(entt, *(std::get<component_iterator<Strong>>(it)++)..., std::get<pool_type<WeakRef> *>(pools)->get(entt)...);
            }
        }
    }
